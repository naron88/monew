...
class NotificationServiceImplTest {
    ...
    void createCommentLikeNotificationTest() {
        ...
        notification = new Notification(user, user.getNickname() + " 님이 나의 댓글을 좋아합니다.",
            ResourceType.COMMENT, comment.getId());
        given(notificationRepository.save(any(Notification.class))).willReturn(notification);

        NotificationDto notificationDto = new NotificationDto(UUID.randomUUID(),
            notification.getCreatedAt(), notification.getUpdatedAt(), notification.isConfirmed(),
            notification.getUser().getId(), notification.getContent(), notification.getType(),
